...
-- Check if all options are currently enabled
local function AreAllOptionsEnabled(ply)
	local clipsEnabled = ply:GetInfoNum("showclips", 0) ~= 0
	local triggersEnabled = ply:GetInfoNum("showtriggers_enabled", 0) == 1
	local propsEnabled = ply:GetInfoNum("showprops", 0) == 1
	
	return clipsEnabled and triggersEnabled and propsEnabled
end

-- Toggle all options at once
local function ToggleAllOptions(ply)
	local allEnabled = AreAllOptionsEnabled(ply)
	
	if allEnabled then
		-- Disable all
		ply:ConCommand("showclips 0")
		ply:ConCommand("showtriggers_enabled 0")
		ply:ConCommand("showprops 0")
	else
		-- Enable all
		ply:ConCommand("showclips 1")
		ply:ConCommand("showtriggers_enabled 1")
		ply:ConCommand("showtriggers_types " .. ShowHidden.ALL_TYPES)
		ply:ConCommand("showprops 1")
	end
end

local CHAT_COMMANDS = {
	[OpenMenu] =        { "showhidden", "clipsmenu", "triggersmenu", "showclipsmenu", "showtriggersmenu", "tm" },
