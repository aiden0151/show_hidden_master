...
local function OpenConfigMenu()
	...
	local toggleAllPan = w:Add("DPanel")
	toggleAllPan:SetPaintBackground(false)
	toggleAllPan:DockMargin(2, 4, 2, 2)
	toggleAllPan:SetHeight(32)
	toggleAllPan:Dock(TOP)

	local toggleAllBtn = toggleAllPan:Add("DButton")
	toggleAllBtn:SetText("#showhidden.toggleall.enable")
	toggleAllBtn:Dock(FILL)
	toggleAllBtn:DockMargin(2, 2, 2, 2)
	...
	local function updateToggleAllButton()
		if areAllOptionsEnabled() then
			toggleAllBtn:SetText("#showhidden.toggleall.disable")
		else
			toggleAllBtn:SetText("#showhidden.toggleall.enable")
		end
	end
	...
	-- Add callbacks to update button when ConVars change
	local function onConVarChange()
		timer.Simple(0.1, updateToggleAllButton)
	end
	
	cvars.AddChangeCallback(cv_enabled:GetName(), onConVarChange, "toggleall_clips")
	cvars.AddChangeCallback(cv_triggers:GetName(), onConVarChange, "toggleall_triggers")
	cvars.AddChangeCallback(cv_props:GetName(), onConVarChange, "toggleall_props")
	
	-- Clean up callbacks when window closes
	local originalOnClose = w.OnClose
	w.OnClose = function()
		cvars.RemoveChangeCallback(cv_enabled:GetName(), "toggleall_clips")
		cvars.RemoveChangeCallback(cv_triggers:GetName(), "toggleall_triggers")
		cvars.RemoveChangeCallback(cv_props:GetName(), "toggleall_props")
...
-- Console command for opening the menu (alias)
concommand.Add("tm", OpenConfigMenu, nil, "Open PlayerClips and ShowTriggers settings menu")
...
end, nil, "Toggle all ShowHidden options at once")
